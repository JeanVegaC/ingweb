...
function GetStringHeight($width, $text, $fontFamily, $fontSize) {
    $pdf = new FPDF();
    $pdf->SetFont($fontFamily, '', $fontSize);

    $lineHeight = $pdf->FontSize * 1.5; // Altura estimada de línea

    $textWidth = $width - 2 * $pdf->cMargin; // Ancho disponible para el texto
    ...
    foreach ($textLines as $line) {
        $lineHeight = $pdf->GetStringWidth($line) / $textWidth; // Altura necesaria para la línea actual
